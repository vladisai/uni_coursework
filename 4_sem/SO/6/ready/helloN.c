...
static struct chardriver hello_tab = {
    .cdr_open = hello_open, .cdr_close = hello_close, .cdr_read = hello_read, .cdr_write = hello_write
};

/** State variable to count the number of times the device has been opened.
 * Note that this is not the regular type of open counter: it never decreases.
 */
static char s[DEVICE_SIZE + 1];
...
static int sef_cb_lu_state_save(int UNUSED(state)) {
    /* Save the state. */
    ds_publish_mem("hello_str", s, DEVICE_SIZE, DSF_OVERWRITE);
    return OK;
}
...
static int lu_state_restore() {
    ...
    *len = DEVICE_SIZE;
    ds_retrieve_mem("hello_str", s, len);
    ds_delete_mem("hello_str");
    s[DEVICE_SIZE] = 0;
    return OK;
}
