...
abstract class Operation {
    ...
    public synchronized void waitAndPerform() throws InterruptedException {
        if (!isReady())
            wait();
        perform();
...
class MoveOperation extends Operation {
    ...
    public static boolean isVisited(MoveOperation operation) {
        synchronized (moveOperations) {
            for (MoveOperation moveOperation : visited) {
    ...
    public static boolean areConnected(MoveOperation move1, MoveOperation move2) {
        if (move1.newShape.intersects(move2.shape) || move2.shape.intersects(move1.shape)) {
            return true;
        } else {
            return false;
        }
